...
# DATA_DIR = './data/'
DATA_DIR = '/home/ubuntu/Code/hjl/SemiCD_old/SemiCD-VL/SemiCD-VL-main/data/'
...
def human_format(num):
    ...
    while abs(num) >= 1000:
        magnitude += 1
        num /= 1000.0
    return '{}{}'.format('{:f}'.format(num).rstrip('0').rstrip('.'), ['', 'K', 'M', 'B_2', 'T'][magnitude])
...
def config_from_vars(
    ...
    vl_label_root_whu=None,
    vl_change_label_root_whu=None,
    ...
    if method in ['diffmatch_unimatch', 'diffmatch_fixmatch', 'diffmatch_unimatch_hjl']:
        cfg['use_fp'] = use_fp
        if not use_fp:
            name += '-nfp'
    ...
    if disable_dropout:
        name += '-disdrop'
    if method in ['diffmatch', 'diffmatch_unimatch', 'diffmatch_fixmatch','diffmatch_unimatch_hjl',]:
        cfg['pleval'] = pleval
        if pleval:
            name += '-plev'
    cfg['fp_rate'] = fp_rate
    ...
    # 动态设置每卡 batch size
    # if dataset == 'levir':
    #     cfg['batch_size'] = 8 if n_gpus == 1 else 4
    # elif dataset == 'whu':
    #     cfg['batch_size'] = 16 if n_gpus == 1 else 8
    # else:
    #     cfg['batch_size'] = batch_size  # 默认不变
    cfg['batch_size'] = batch_size
    if n_gpus != 4 or cfg['batch_size'] != 2 or n_nodes != 1:
        name += f'_{n_nodes}x{n_gpus}x{cfg["batch_size"]}'
    ...
    cfg['vl_label_root_whu'] = vl_label_root_whu
    cfg['vl_change_label_root_whu'] = vl_change_label_root_whu
...
def generate_experiment_cfgs(exp_id,ngpus=1):
    cfgs = []
    # -------------------------------------------------------------------------
    ...
    if exp_id == 48:
        ...
        splits = ['5%', '10%', '20%', '40%']
        list_kwargs = [
            dict(model='mmseg.vlm-lighthead-r50', opt='original', lr=5.5e-3, backbone_lr_mult=20., criterion='CELoss',
                 ...
                 # vl_label_root='ape_output/whu_30/whu_cd_pseudo_label_030',
                 # vl_change_label_root='ape_output/whu_30/whu_self_train_pixel_level_change_mask_30',
                 contrastive_loss_weight=0.1)]
        for split, kwargs, _ in itertools.product(splits, list_kwargs, range(n_repeat)):
            cfg = config_from_vars(
                exp_id=exp_id,
                dataset='whu',
                method='diffmatch_unimatch',
                n_gpus=ngpus,
                split=str(split),
                conf_thresh=0.95,
                crop_size=256,
    ...
    # -------------------------------------------------------------------------
        ...
        list_kwargs = [
            dict(model='mmseg.vlm-lighthead-r50', opt='original', lr=1e-2, backbone_lr_mult=20., criterion='CELoss',
                 vl_consistency_lambda=[0.1, 0], vl_loss_reduce='mean_all',
                 # vl_label_root='ape_output/levir_25_2/levir_cd_pseudo_label_025_2',
                 # vl_change_label_root='ape_output/levir_25_2/levir_self_train_pixel_level_change_mask_25_2',
                 ...
                 # vl_label_root_whu='ape_output/gen_seg_label/ape_selftrain_whu_cd_pseudo_label',
                 # vl_change_label_root_whu='ape_output/gen_cd_label/whu_self_train_pixel_level_change_mask',
                 contrastive_loss_weight=0.1)]
        for split, kwargs, _ in itertools.product(splits, list_kwargs, range(n_repeat)):
            cfg = config_from_vars(
                exp_id=exp_id,
...
def save_experiment_cfgs(exp_id,ngpus=1):
    cfgs = generate_experiment_cfgs(exp_id,ngpus)
    cfg_files = []
    for cfg in cfgs:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ngpus', type=int, default=1, help='Override number of GPUs')
    args = parser.parse_args()

    cfgs, cfg_files = save_experiment_cfgs(args.exp,args.ngpus)
