...
def predict(model, imgA, imgB, mask, mode, cfg, return_logits=False):

    final,_,segA,segB = model(imgA, imgB,need_seg_aux=True)
    pred = final.argmax(dim=1)
    segA = segA.argmax(dim=1)
    segB = segB.argmax(dim=1)
        
    if return_logits:
        return pred, final
    else:
        return pred,segA,segB
...
def save_pred_mask(pred, save_dir, name):
    os.makedirs(save_dir, exist_ok=True)

    # 转成 numpy 数组，uint8 格式（0 和 1）
    pred_np = pred.squeeze().cpu().numpy().astype(np.uint8) * 255  # 如果是二分类，1→255
...
def evaluate2(model, loader, mode, cfg, return_cd_metric=False):
    model.eval()
    ...
    with torch.no_grad():
        for imgA, imgB, mask, id in tqdm(loader, total=len(loader)):
            imgA = imgA.cuda()
            imgB = imgB.cuda()

            pred,segA,segB = predict(model, imgA, imgB, mask, mode, cfg)
            ...
            if dist.get_rank() == 0:  # 只在主进程保存
                save_pred_mask(pred[0], save_dir="./pred_whu_SemiCD-VL_CD", name=id[0])
    ...
    # print(intersection_meter.sum, union_meter.sum, correct_pixel.sum, output_meter.sum, target_meter.sum)
    f1_class = [f_score(x[0], x[1], 1) * 100.0 for x in zip(precision, recall)]

    if return_cd_metric:
        return mIOU, iou_class, overall_acc, f1_class, precision * 100, recall * 100
    return mIOU, iou_class
def evaluate(model, loader, mode, cfg, return_cd_metric=False):
    model.eval()
    assert mode in ['original', 'center_crop', 'padded_sliding_window', 'zegclip_sliding_window', 'sliding_window']
    intersection_meter = AverageMeter()
    union_meter = AverageMeter()
    ...
    with torch.no_grad():
        for imgA, imgB, mask, id in tqdm(loader, total=len(loader)):
            ...
            pred,_,_ = predict(model, imgA, imgB, mask, mode, cfg)
