...
class SemiBCDDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # imgA = Image.open(os.path.join(self.root, 'A_2', id)).convert('RGB')
        # imgB = Image.open(os.path.join(self.root, 'B_2', id)).convert('RGB')
        #
        ...
        try:
            ...
            print(f"Error loading vl {id}: {e}")
            return None  # 跳过或返回默认值
        ####################
        vl_mask_A = np.array(vl_mask_A)
        vl_mask_B = np.array(vl_mask_B)
        vl_mask_change = np.array(vl_mask_change)
        
        vl_mask_A = (vl_mask_A >= 128).astype(np.uint8)
        vl_mask_B = (vl_mask_B >= 128).astype(np.uint8)
        vl_mask_change = (vl_mask_change >= 128).astype(np.uint8)
        
        # 转为 PIL Image 后再转换为 tensor
        vl_mask_A = Image.fromarray(vl_mask_A)
        vl_mask_B = Image.fromarray(vl_mask_B)
        vl_mask_change = Image.fromarray(vl_mask_change)
        #####################
