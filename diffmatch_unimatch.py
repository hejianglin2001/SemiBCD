...
from utils.train_utils import DictAverageMeter
...
if __name__ == '__main__':
    ...
    trainset_u = SemiBCDDataset(cfg, 'train_u', id_path=unlabeled_id_path, isMix=False)
    trainset_l = SemiBCDDataset(cfg, 'train_l', id_path=labeled_id_path, nsample=len(trainset_u.ids))
    valset = SemiBCDDataset(cfg, 'val')
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((imgA_x, imgB_x, mask_x, vl_mask_x_change, vl_mask_x_A, vl_mask_x_B),
            ...
            pred_fp, pred_fp_for_vl, pred_fp_segA, pred_fp_segB, pred_fp_dist, feat_w_A, feat_w_B = model(imgA_w,
                                                                                                          imgB_w,
                                                                                                          need_seg_aux=True,
                                                                                                          need_contrast=True,
                                                                                                          need_fp=True)
            ...
            pred_s, pred_s_for_vl, pred_s_segA, pred_s_segB, pred_s_dist, feat_s_A, feat_s_B = model(imgA_s1, imgB_s1,
                                                                                                     need_seg_aux=True,
                                                                                                     need_contrast=True)
            ...
            lp2w = (loss_w_segA + loss_w_segB) * (1/4) + loss_w_cd * 4.5
            ...
            # L_mvc
            loss_mvc = (lw2f + lw2i + lp2w)

            # 计算 Lsc 损失
            lsc_s_cd = compute_lsc_loss(pred_s_segA, pred_s_segB, vl_mask_change, m_u=m_u, m_c=m_c)
            lsc_w_cd = compute_lsc_loss(pred_w_segA, pred_w_segB, vl_mask_change, m_u=m_u, m_c=m_c)
            lsc_x_cd = compute_lsc_loss(pred_x_segA, pred_x_segB, mask_x, m_u=m_u, m_c=m_c)
            loss_lsc = (lsc_s_cd + lsc_w_cd + lsc_x_cd * 1.5) * (1/3)
            # L_fc
            lfc_x = compute_lfc_loss(feat_x_A, feat_x_B, mask_x)
            lfc_w = compute_lfc_loss(feat_w_A, feat_w_B, vl_mask_change)
            lfc_s = compute_lfc_loss(feat_s_A, feat_s_B, vl_mask_change)
            loss_lfc = (lfc_x * 1.5 + lfc_w + lfc_s) * (1/3)
            # L_tha
            l_tha = (loss_lfc + loss_lsc) * 0.5

            loss_supervised = (loss_x_segA + loss_x_segB) * (1/12) + (loss_x * 2.0)
#12

            loss = loss_supervised + (1/3) * l_tha + (1/3) * loss_mvc
            # loss = loss_supervised + (0.7) * l_tha + (0.7) * loss_mvc
